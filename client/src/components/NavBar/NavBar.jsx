  ...
  return (
    <>
      <div className="flex bg-slate-100  h-10 justify-evenly fixed w-full z-20 top-0 start-0">
        ...
        <nav className="bg-slate-300 flex mx-20 items-center w-full">
          {props.routeReference.map((item, i) => (
            <Link
              key={i}
              className="m-auto mx-1 h-full content-center w-full duration-300 hover:cursor-pointer hover:bg-slate-500 hover:text-white"
              activeClass="active"
              to={item.id}
              spy={true}
              smooth={true}
